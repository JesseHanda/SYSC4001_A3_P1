/**
 * @file interrupts_101264747_101276213_EP.cpp
s * @author Jesse Handa
...
static void dispatch_highest_priority(
    ...
    if (iterator1 == ready_queue.end()) return; // nothing available now
    PCB next = *iterator1;
...
std::tuple<std::string> run_simulation(std::vector<PCB> list_processes) {
    ...
    while (!all_process_terminated(job_list) || job_list.empty()) {
        ...
        if (running.PID != -1) {
            ...
            sync_queue(job_list, running);
            if (running.remaining_time == 0) { // when finished
            ...
            // if it needs IO: running -> waiting
